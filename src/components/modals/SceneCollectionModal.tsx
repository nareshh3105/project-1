...
import { X, Check, Plus, Copy, Edit2, Trash2, Download, Upload, Layers } from 'lucide-react'
import { useUIStore } from '@/stores/uiStore'
import { useCollectionStore, type CollectionItem } from '@/stores/collectionStore'
import { RenameModal } from './RenameModal'
import { ConfirmModal } from './ConfirmModal'
import { cn } from '@/lib/utils'

export function SceneCollectionModal() {
  const modal      = useUIStore((s) => s.modal)
  const closeModal = useUIStore((s) => s.closeModal)
  const isOpen = modal?.type === 'scene-collection'

  const {
    collections, activeCollectionId,
    createCollection, renameCollection, deleteCollection,
    duplicateCollection, switchCollection, exportCollection, importCollection,
  } = useCollectionStore()
  ...
  return (
    <>
        ...
        <Dialog.Portal>
          ...
          <Dialog.Content
            className="fixed z-50 top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2
              w-[480px] max-h-[520px] flex flex-col bg-bg-surface
            ...
            <div className="flex-1 overflow-y-auto">
              {collections.length === 0 ? (
                <div className="flex flex-col items-center justify-center h-32 gap-2">
                  <Layers size={20} className="text-text-muted opacity-30" />
                  <p className="text-caption text-text-muted">No collections yet</p>
                </div>
              ) : (
                collections.map((c) => (
                  <CollectionRow
                    ...
                    onExport={() => exportCollection(c.id)}
      ...
      <RenameModal
        ...
        onConfirm={(name) => renameTarget && renameCollection(renameTarget.id, name)}
        onClose={() => setRenameTarget(null)}
      />
      <ConfirmModal
        open={!!deleteTarget}
        title="Delete Scene Collection"
        message={`Delete "${deleteTarget?.name}"? All scenes and sources in it will be permanently removed.`}
        confirmLabel="Delete"
        danger
...
function CollectionRow({
  collection, isActive, onSwitch, onRename, onDuplicate, onExport, onDelete, canDelete,
}: {
  ...
  return (
    ...
    >
      ...
      <span className="flex-1 text-body truncate">{collection.name}</span>

      <div className="flex items-center gap-0.5 opacity-0 group-hover:opacity-100 transition-opacity">
        <RowIconButton title="Export" onClick={onExport}><Download size={12} /></RowIconButton>
